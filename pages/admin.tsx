...
type Tab = 'overview' | 'members' | 'donations' | 'organizations' | 'events' | 'posts' | 'gallery';
...
export default function AdminPage() {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      setActionError('Failed to load events.');
    } finally {
      setTabLoading(false);
    }
  }, []);
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      setActionError('Failed to load posts.');
    } finally {
      setTabLoading(false);
    }
  }, []);
  ...
  const fetchGallery = useCallback(async () => {
    ...
    try {
      ...
      setActionError('Failed to load gallery.');
    } finally {
      setTabLoading(false);
    }
  }, []);
  ...
  }, [activeTab, user, membersLoaded, eventsLoaded, postsLoaded, galleryLoaded, fetchMembers, fetchEvents, fetchPosts, fetchGallery]);
            ...
            {tabLoading ? <TabSkeleton /> : (
              <div className="table-container">
                <table className="table">
                  <thead>
                    <tr>
                      <th>Name</th><th>Email</th><th>Level</th><th>Rating</th><th>Status</th><th>Joined</th><th>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredMembers.map(m => (
                      <tr key={m.id}>
                        ...
                        <td>
                          <span className={`badge text-xs ${m.status === 'ACTIVE' ? 'badge-green' : m.status === 'SUSPENDED' ? 'badge-red' : 'badge-gold'}`}>
                            {m.status}
                          </span>
                        </td>
                        ...
                        <td>
                          <div className="flex gap-2">
                            {m.status !== 'ACTIVE' && (
                              <button onClick={() => handleStatusChange(m.id, 'ACTIVE')} className="text-green-400 hover:text-green-300 text-xs transition-colors">Approve</button>
                            )}
                            {m.status !== 'SUSPENDED' && (
                              <button onClick={() => handleStatusChange(m.id, 'SUSPENDED')} className="text-red-400 hover:text-red-300 text-xs transition-colors">Suspend</button>
                            )}
                          </div>
                        </td>
                      </tr>
        ...
        {activeTab === 'donations' && (
          <div>
            ...
            <div className="table-container">
              <table className="table">
                <thead>
                  <tr><th>Donor</th><th>Amount</th><th>Category</th><th>Status</th><th>Date</th></tr>
                </thead>
                <tbody>
                  {donations.map(d => (
                    <tr key={d.id}>
        ...
        {activeTab === 'organizations' && (
          <div>
            ...
            <div className="table-container">
              <table className="table">
                <thead>
                  <tr><th>Organization</th><th>Type</th><th>Location</th><th>Contact</th><th>Members</th><th>Status</th><th>Actions</th></tr>
                </thead>
                <tbody>
                  {orgs.map(org => (
                    <tr key={org.id}>
          ...
          <div>
              ...
              <form onSubmit={handleCreateEvent} className="glass p-6 rounded-xl mb-6 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required className={INP} value={eventForm.title} onChange={e => setEventForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. Monthly Rapid Tournament" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input required className={INP} value={eventForm.location} onChange={e => setEventForm(f => ({ ...f, location: e.target.value }))} placeholder="e.g. Nairobi Chess Center" />
                  </div>
                  <div>
                    <label className={LBL}>Capacity</label>
                    <input type="number" min={1} className={INP} value={eventForm.capacity} onChange={e => setEventForm(f => ({ ...f, capacity: parseInt(e.target.value) || 32 }))} />
                  </div>
                  <div>
                    <label className={LBL}>Start Date & Time *</label>
                    <input required type="datetime-local" className={INP} value={eventForm.startDate} onChange={e => setEventForm(f => ({ ...f, startDate: e.target.value }))} />
                  </div>
                  <div>
                    <label className={LBL}>End Date & Time *</label>
                    <input required type="datetime-local" className={INP} value={eventForm.endDate} onChange={e => setEventForm(f => ({ ...f, endDate: e.target.value }))} />
                  </div>
                </div>
                <div>
                  <label className={LBL}>Description *</label>
                  <textarea required className={`${INP} h-24 resize-none`} value={eventForm.description} onChange={e => setEventForm(f => ({ ...f, description: e.target.value }))} placeholder="Describe the event…" />
                </div>
                <div className="flex justify-end gap-3">
                  <button type="button" onClick={() => setShowEventForm(false)} className="btn-ghost text-sm px-4 py-2 rounded-xl">Cancel</button>
                  <button type="submit" disabled={formSubmitting} className="btn-primary text-sm px-6 py-2 rounded-xl">
            ...
            {tabLoading ? <TabSkeleton rows={3} /> : (
              <div className="table-container">
                <table className="table">
                  <thead>
                    <tr><th>Event</th><th>Type</th><th>Location</th><th>Date</th><th>Registrations</th><th>Actions</th></tr>
                  </thead>
                  <tbody>
                    {events.map(ev => (
                      <tr key={ev.id}>
                        ...
                        <td>
                          <button onClick={() => handleDeleteEvent(ev.id)} className="text-red-400 hover:text-red-300 text-xs transition-colors">Delete</button>
                        </td>
                      </tr>
                    ))}
                    {events.length === 0 && (
                      <tr><td colSpan={6} className="text-center py-8 text-gray-500">No events yet — create one above</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
        ...
        {activeTab === 'posts' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowPostForm(v => !v)} className="btn-primary text-sm px-4 py-2 rounded-xl">
              ...
              <form onSubmit={handleCreatePost} className="glass p-6 rounded-xl mb-6 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    <label className={LBL}>Title *</label>
                    <input required className={INP} value={postForm.title} onChange={e => setPostForm(f => ({ ...f, title: e.target.value }))} placeholder="Post title" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input className={INP} value={postForm.imageUrl} onChange={e => setPostForm(f => ({ ...f, imageUrl: e.target.value }))} placeholder="https://…" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className={LBL}>Tags (comma-separated)</label>
                    <input className={INP} value={postForm.tags} onChange={e => setPostForm(f => ({ ...f, tags: e.target.value }))} placeholder="chess, tournament, kenya" />
                  </div>
                </div>
                <div>
                  <label className={LBL}>Excerpt *</label>
                  <textarea required className={`${INP} h-20 resize-none`} value={postForm.excerpt} onChange={e => setPostForm(f => ({ ...f, excerpt: e.target.value }))} placeholder="Short summary shown in listings…" />
                </div>
                <div>
                  <label className={LBL}>Content *</label>
                  <textarea required className={`${INP} h-48 resize-y`} value={postForm.content} onChange={e => setPostForm(f => ({ ...f, content: e.target.value }))} placeholder="Full post content…" />
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button type="submit" disabled={formSubmitting} className="btn-primary text-sm px-6 py-2 rounded-xl">
                      {formSubmitting ? 'Saving…' : 'Save Post'}
                    </button>
                  </div>
                </div>
              </form>
                  ...
                  <tbody>
                    {posts.map(post => (
                      <tr key={post.id}>
                        ...
                        <td>
                          <span className={`badge text-xs ${post.published ? 'badge-green' : 'badge-gray'}`}>
                    ...
                    {posts.length === 0 && (
                      <tr><td colSpan={6} className="text-center py-8 text-gray-500">No posts yet — write one above</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
        ...
        {activeTab === 'gallery' && (
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowGalleryForm(v => !v)} className="btn-primary text-sm px-4 py-2 rounded-xl">
                {showGalleryForm ? '✕ Cancel' : '+ Add Photo'}
              </button>
            </div>
            ...
            {showGalleryForm && (
              <form onSubmit={handleAddGalleryItem} className="glass p-6 rounded-xl mb-6 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className={LBL}>Title *</label>
                    <input required className={INP} value={galleryForm.title} onChange={e => setGalleryForm(f => ({ ...f, title: e.target.value }))} placeholder="Photo title" />
                  </div>
                  <div>
                    <label className={LBL}>Caption</label>
                    <input className={INP} value={galleryForm.caption} onChange={e => setGalleryForm(f => ({ ...f, caption: e.target.value }))} placeholder="Optional caption" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className={LBL}>Image URL *</label>
                    <input required className={INP} value={galleryForm.imageUrl} onChange={e => setGalleryForm(f => ({ ...f, imageUrl: e.target.value }))} placeholder="https://…" />
                  </div>
                </div>
                <div className="flex justify-end gap-3">
                  <button type="button" onClick={() => setShowGalleryForm(false)} className="btn-ghost text-sm px-4 py-2 rounded-xl">Cancel</button>
                  <button type="submit" disabled={formSubmitting} className="btn-primary text-sm px-6 py-2 rounded-xl">
                    {formSubmitting ? 'Adding…' : 'Add Photo'}
                  </button>
                </div>
              </form>
            )}
                ...
                {galleryItems.map(item => (
                  <div key={item.id} className="glass rounded-xl overflow-hidden group relative">
                    ...
                    <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-1 p-3">
                      ...
                      <button onClick={() => handleDeleteGalleryItem(item.id)} className="mt-2 text-red-400 hover:text-red-300 text-xs border border-red-400/30 rounded px-2 py-1 transition-colors">
                        Delete
                      </button>
                    </div>
                  </div>
