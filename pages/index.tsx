            ...
            <div className="p-8 flex flex-col justify-center">
              ...
              {!revealed ? (
                <div className="space-y-3">
                  <Link href="/puzzles" className="block btn-primary text-center">Solve the Puzzle →</Link>
                  <button onClick={() => setRevealed(true)} className="w-full btn-secondary text-sm py-2">Peek at Position</button>
                </div>
              ) : (
                <Link href="/puzzles" className="btn-primary text-center block">Solve on Puzzle Page →</Link>
              )}
            </div>
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-yellow-900/60 via-yellow-800/30 to-yellow-900/60 border-b border-yellow-500/30">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-11 h-11 rounded-xl bg-yellow-500/25 border border-yellow-500/50 flex items-center justify-center text-2xl animate-pulse-gold flex-shrink-0">♛</div>
            <div className="text-center sm:text-left">
              <p className="text-white font-bold text-base leading-tight">Support Zaid Knights</p>
              <p className="text-yellow-200/60 text-sm mt-0.5">Help fund tournaments, training, and player development in Kenya</p>
            </div>
          </div>
          <Link href="/donate" className="flex-shrink-0 bg-yellow-500 hover:bg-yellow-400 active:bg-yellow-600 text-black font-bold text-sm px-8 py-3 rounded-full transition-all duration-200 shadow-lg shadow-yellow-500/40 hover:shadow-yellow-400/50 hover:scale-105 whitespace-nowrap">
            Donate Now →
          </Link>
        </div>
      </div>
      ...
      <section className="py-20">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <div className="glass-gold p-12 rounded-2xl">
            ...
            <h2 className="text-4xl font-bold text-white mb-4" style={{ fontFamily: 'Playfair Display, serif' }}>Join Zaid Knights Today</h2>
            <p className="text-gray-400 mb-8">Become part of Nairobi's fastest-growing chess community. Free to start, no experience needed.</p>
